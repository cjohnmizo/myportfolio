...
export function SiteFooter({ footerNote }: { footerNote: string }) {
  return (
    <footer className="border-border relative overflow-hidden border-t py-10">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-end lg:px-8">
        ...
        <div className="flex flex-col gap-4 lg:items-end">
          <div className="text-muted-foreground flex flex-wrap items-center gap-3 text-sm lg:justify-end">
            {siteConfig.navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="border-border hover:border-primary/40 hover:text-primary rounded-full border px-3 py-1.5 transition"
              >
                {item.label}
              </Link>
            ))}
          ...
          <Button asChild variant="outline">
            <Link href="/contact">Open transmission terminal</Link>
          </Button>
        </div>
      </div>
